...
type EventRegistrationWithDetails = Tables<'event_registrations'> & {
  events: { id: string; title: string; description: string; date: string; location: string; ngo_id: string };
...
type ConfirmedVolunteerWithDetails = NgoEnrollmentWithDetails;
...
export function NgoDashboard() {
  ...
  const handleVolunteerAction = async (volunteerId: string, volunteerName: string) => {
    try {
      const { error } = await supabase.from('ngo_enrollments').update({ status: 'rejected' }).eq('id', volunteerId);
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex border-b mb-6">
          {[
            ...
            <button
              key={tab.key}
              className={`px-4 py-2 font-medium ${currentTab === tab.key ? 'border-b-2 border-rose-600 text-rose-600' : 'text-gray-500'}`}
                        ...
                        <div className="space-y-2">
                          {event.participants.map((p) => (
                            <div
                              ...
                              className="flex justify-between items-center bg-white p-2 border rounded text-sm"
                            >
                              <div>
          ...
          <div className="space-y-3">
            {requests.eventRegistrations.length === 0 ? (
              ...
              requests.eventRegistrations.map((reg) => (
                <div
                  key={reg.id}
                  className="bg-white p-4 rounded-lg shadow-sm border border-l-4 border-l-rose-500"
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex-shrink-0">{renderActionButtons('event', reg.id)}</div>
                  </div>
                </div>
              ))
            )}
          ...
          <div className="space-y-3">
            {requests.ngoEnrollments.length === 0 ? (
              ...
              requests.ngoEnrollments.map((enroll) => (
                <div
                  key={enroll.id}
                  className="bg-white p-4 rounded-lg shadow-sm border border-l-4 border-l-rose-400"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex-1 min-w-0">
                    ...
                    <div className="flex-shrink-0">{renderActionButtons('ngo', enroll.id)}</div>
                  </div>
                </div>
              ))
            )}
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              ...
              <div className="text-sm text-gray-500">
                {requests.confirmedVolunteers.length} active volunteer
                {requests.confirmedVolunteers.length !== 1 ? 's' : ''}
              </div>
            </div>
            ...
            {requests.confirmedVolunteers.length === 0 ? (
              ...
              <div className="space-y-3">
                {requests.confirmedVolunteers.map((volunteer) => (
                  <div
                    key={volunteer.id}
                    className="bg-white p-4 rounded-lg shadow-sm border border-l-4 border-l-green-500"
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center space-x-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center mt-1">
                              ...
                              <span className="text-xs text-gray-500 ml-2">Joined: {formatDate(volunteer.created_at)}</span>
                            </div>
                          </div>
                        </div>
                      </div>
